...
// Function prototypes
void inputProcesses(int n, int proc_id[], int at[], int bt[], int b[]);
void calculateCompletionTime(int n, int at[], int b[], int ct[], int m[]);
...
int main() {
    ...
    int proc_id[n], at[n], bt[n], b[n], ct[n], tat[n], wt[n], m[n];
    ...
    inputProcesses(n, proc_id, at, bt, b);

    // Calculate completion time
    calculateCompletionTime(n, at, b, ct, m);
...
void calculateCompletionTime(int n, int at[], int b[], int ct[], int m[]) {
    int c = 0, count = 0, min_bt_index, min_bt, p = 0;

    while (count < n) {
        min_bt = 1e9; // large value to ensure it gets overwritten

        // Find the process with the smallest remaining burst time that has arrived and is not completed
        for (int i = 0; i < n; i++) {
            if (at[i] <= c && m[i] != 1 && b[i] < min_bt) {
                min_bt = b[i];
                min_bt_index = i;
            }
        }
        ...
        // If the process is completed, mark it as completed and record its completion time
        if (b[min_bt_index] == 0) {
            m[min_bt_index] = 1;
            ct[min_bt_index] = c;
            count++;
